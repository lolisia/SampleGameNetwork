...
// buildings are placed in stategy games. Objects will always be placed with
// the bottom touching the grid. Mouse input is being handled by shooting a ray
// from the cursor and letting it hit the grid's collider. If there is no
// collider nothing will happen. In this example I used a plane collider on the
// grid.
// 
...
// intersecting with another block. In that case the active block will be
// tinted red until you move out. if you let go while intersecting it will snap
// back to the last non-intersecting position. This is achieved by using a
// child object with a trigger that checks for intersections and reports back
