...
#region  Callback methods
		...
		void OnGUI() {
			...
			var message = "Use arrow keys to scroll the camera, hold shift to scroll faster. " +
				"The current camera position is\n\t" + currX +" / " + currY + "\n " +
				"and the last fixed position was\n\t" + lastX +" / "+ lastY +
				"\n\vThe grid's rendering range is only adjusted when the camera reaches the " +
				"edge of the grid, and that prompts the grid to re-calculate its points. This gives" +
				" us the illusion of a seemingly endless grid while we only render what's within " +
				"close reach. This is cheaper performance-wise than rendering a huge grid when the " +
